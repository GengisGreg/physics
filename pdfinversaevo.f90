program pdfevoinv
integer i,j,k,l,e,d
integer,parameter::n=4096,m=100000,ntot=409600,np=100
...
open(unit=25,file="realeuinversa.dat",status='old')
open(unit=21,file="pdfinvevo.dat")
open(unit=34,file="deltauinv.dat")
...
!do i=1,n
	!	if((i+d).le.ntot) then
	!		deltau(i)=u(i+d)-u(i)
	!	else if((i+d).gt.ntot) then
	!		deltau(i)=u(i+d-ntot)-u(i)
	!	end if
!	end do
mean=0.d0
do j=1,100
